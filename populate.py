...
#this csv.field_size_limit allows us to get around the memory problems caused with the python program dropping out running out of space.
...
def add_part(part_number,description,rrp_price,buy_price):
    p = Part.objects.get_or_create(part_number=part_number, description=description, rrp_price=rrp_price, buy_price=buy_price)[0]
    return p


def find_buy_price(retail_price,discount_code):

    #Trim the discount code as we only need the second letter to work out our discount code letter
    ...
    #Check to make sure tha the discount code is one letter - otherwise report an error
...
for line in f.xreadlines():
    ...
    #A small test to make sure that the discount code is valid and not empty or too small
    if len(discount_code) == 0 or len(discount_code) >= 3:
        print "Error Found, discount code is either too short or too big"
        #error report into the file (or another file so that we know that this part is invalid
    ...
    output_file.writerow([part_number,part_description,retail_price,quantity_issued_in,discount_code,unknown,surcharge,buy_price])
